...
void main()
{
	...
	*/
	float x = max(dot(normalize(normal), normalize(light_direction)), 0.0);
	float y = pow(max(dot(normalize(normal), normalize(light_direction + view_direction)), 0.0), material_shininess);
	vec3 color = material_color * (material_ambient + light_color * (x + y));
